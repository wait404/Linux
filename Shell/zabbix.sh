...
chinese_font_file=https://github.com/Halfish/lstm-ctc-ocr/raw/master/fonts/simkai.ttf
...
function DB_root_password()
{
    while true
    do
        read -p "请输入mysql root密码：" database_root_password
...
function DB_zabbix_password()
{
    while true
    do
        read -p "请输入zabbix的数据库密码：" database_zabbix_password
...
function Install_zabbix()
{
    ...
    mysql -u zabbix -p$database_zabbix_password zabbix < $src_path/zabbix-3.0.25/database/mysql/schema.sql
    mysql -u zabbix -p$database_zabbix_password zabbix < $src_path/zabbix-3.0.25/database/mysql/images.sql
    mysql -u zabbix -p$database_zabbix_password zabbix < $src_path/zabbix-3.0.25/database/mysql/data.sql
    ...
    mv $src_path/cat zabbix-3.0.25/frontends/php/* $zabbix_web_path
...
{
    if [ -n "$check_zabbix_file" ]
    then
        service zabbix_server stop
        service zabbix_agebtd stop
